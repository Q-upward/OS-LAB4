...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    // 1. 调用alloc_proc分配一个proc_struct结构体
    ...
    // 3. 根据clone_flags调用copy_mm来复制或共享内存管理结构
    // copy_mm目前什么也不做，对于这个实验总是返回0 
    ...
    // 5 & 6. 关键区：PID分配、入队、状态设置 (需要原子操作保护)
    bool intr_flag;
    local_intr_save(intr_flag);
    {
        // 5. 分配PID并加入列表 (确保原子性)
